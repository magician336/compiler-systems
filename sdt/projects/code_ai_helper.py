...
"""
        try:
            with open(os.path.join(PROJECT_DIR, "sdt.log"), "a", encoding="utf-8") as f:
                f.write(log_entry)
        except Exception as e:
            print(f"⚠️ 写入 sdt.log 失败: {e}")
...
def get_system_prompt(exercise, exercises, policies):
    ...
    # 支持多个 ex_file，用逗号分隔
    ex_files_str = ex.get("ex_file", "")
    ex_files = [f.strip() for f in ex_files_str.split(",") if f.strip()]
    if not ex_files:
        return None

    ex_content = ""
    for ex_file in ex_files:
        content = read_file(ex_file)
        if content:
            ex_content += content + "\n"
        else:
            ex_content += f"⚠️ 文件 {ex_file} 未找到\n"

    compile_log_text = compile_log if compile_log else "（无编译日志，可能尚未编译）"
    run_log_text = run_log if run_log else "（无运行日志，可能尚未运行）"
    source_code = (aux_file or "") + ex_content
    ...
    lines = [
        f"你是一个专业的编译系统原理课程助教。学生正在进行语法制导翻译方法的学习，目前是{ex.get('title', exercise)}练习。",
        "",
        "【练习任务说明】",
        task_text,
        "",
...
class AIChat:
    def __init__(self, exercise):
        ...
        initial_msg = "请分析我对习题的解答，指出问题所在并引导我修改。"
